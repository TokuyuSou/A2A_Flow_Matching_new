...
| 対象層 | 指定した Linear 層のみ | 全ての Linear / Conv1d 層 |

### 仕組み

1. Phase-1 の事前訓練済み重み `W_base` を凍結してロード
2. 全ての `nn.Linear` / `nn.Conv1d` 層に対して、差分 `delta` を学習
...
`obs_encoder`（凍結 ResNet）以外の全ての `nn.Linear` および `nn.Conv1d`:

- `flow_net` 内の全 Linear 層（input_proj, time_embed, cond_embed, task_proj, FlowNetLayer 内の MLP・modulator, out_proj）
- `action_decoder` 内の全 Linear 層（input_proj, MLP layers, output_proj）
- `obs_projector`
- `action_encoder` / `history_action_encoder` 内の全 Conv1d 層 + latent_proj
